...
class  WikiRequests():
    ...
    async def get_full_text_from_page(self, page_ident: str) -> str:
        logger.info(f"Trying to get full text of page: '{page_ident}' from {self.wiki_url}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url=self.wiki_url, params=request_params) as response:
                if response.status != 200:
                        logger.error(f"Failed to get page property: {response.status}")
                        return ""
    ...
    async def get_short_text_from_page(self, page_ident: str) -> str:
        logger.info(f"Trying to get short page text : '{page_ident}' from {self.wiki_url}")
    ...
    async def get_search_results(self, searched_string: str) -> SearchResponse:
        logger.info(f"Trying to get search results of: '{searched_string}' from {self.wiki_url}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url=self.wiki_url, params=request_params) as response:
                if response.status != 200:
                        logger.error(f"Failed to get search results: {response.status}")
                        return []
        ...
        return SearchResponse(
            ...
            short_descriptions=json_response[2],
